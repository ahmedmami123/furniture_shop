...
<div className="profil">
  <div className="sel">
    <div onClick={()=>{
      setproduct(true) 
      }}  className={!product? "div" : "div1"}>Product List</div>
    <div onClick={()=>{
      setproduct(false) 
    }}  className={!product? "div1" : "div"}>Users List</div>
  ...
  <input placeholder='Product name' type="text" onChange={(e)=>setshopp({...shopp,Product_name:e.target.value})}></input>
...
:<div className="users">
    ...
    <option value="Admin">Admin</option>
      ...
      <div className='bottom'>
        {
  userr?.filter((el)=>el.option==search1 && (el.name.toLowerCase().includes(search.toLowerCase()) || el.Lastname.toLowerCase().includes(search.toLowerCase()))).map((el)=><UserCard el={el} ping={ping} setPing={setPing}/>)
